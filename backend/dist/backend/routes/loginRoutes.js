...
const sendRegistrationEmail = (userEmail, passToken) => emailSender(userEmail, registerSubj, "Verification Link", BACKEND_BASE_URL, "verify", passToken);
router.post("/signup", async (req, res, next) => {
    ...
    const pass = req.body.password.trim().normalize("NFC");
    const hash = await bcrypt.hash(pass, 12);
    let token = crypto.randomBytes(32).toString("hex");
    try {
        //check for email uniqueness in both tables and non-expired emails  --- DONE
        const existingUnverifiedEmail = await db.query("SELECT * FROM unverified_users WHERE email=$1 AND expires_at > NOW()", [req.body.email]);
        const existingActiveEmail = await db.query("SELECT * FROM users WHERE email=$1", [req.body.email]);
...
router.get("/verify", async (req, res, next) => {
    ...
    if (token) {
        try {
            const checkExpiration = await db.query("SELECT * FROM unverified_users WHERE token=$1 AND expires_at > NOW()", [token]);
            if (checkExpiration.rows.length === 0) {
                ...
                try {
                    ...
                    await client.query("INSERT INTO users (email, password, first_name, last_name) SELECT email, password, first_name, last_name FROM unverified_users WHERE token=$1", [token]);
...
router.post("/resend-verification", async (req, res, next) => {
    try {
        const existingUnverifiedUser = await db.query("SELECT * FROM unverified_users WHERE email=$1 AND expires_at > NOW()", [req.body.email]);
