...
router.get("/vacation:id", ensureLoggedIn, async (req, res, next) => {
  try {
    const result = await db.query(
      "SELECT * FROM user_trips WHERE user_id=$1 AND trips_id=$2",
...
router.post("/schedule:id", ensureLoggedIn, async (req, res, next) => {
  try {
    const match = await db.query(
      "SELECT * FROM user_trips WHERE user_id=$1 AND trips_id=$2 AND (role=$3 OR role=$4)",
    ...
    const result = await db.query(
      "INSERT INTO trip_schedule (start_time, end_time, location, cost, details, multi_day) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *",
      [
        req.body.start,
        req.body.end,
        req.body.location,
