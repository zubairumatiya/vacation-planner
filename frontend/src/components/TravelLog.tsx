...
// Eye SVGs
const EyeOpen = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem">
    <path fillRule="evenodd" clipRule="evenodd"
      d="M22 12.0002C20.2531 15.5764 15.8775 19 11.9998 19C8.12201 19 3.74646 15.5764 2 11.9998"
      stroke="#4a38ee" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path fillRule="evenodd" clipRule="evenodd"
...
const EyeHalf = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem">
    <path d="M2 12c1.746-3.576 6.123-7 10-7 3.878 0 8.253 3.424 10 7-1.747-.4-6.122-1-10-1-3.877 0-8.254.6-10 1" fill="#4a38ee"/>
    <path d="M22 12c-1.747 3.576-6.122 7-10 7s-8.254-3.424-10-7" stroke="#4a38ee"/>
    <path d="M22 12c-1.747-3.576-6.122-7-10-7S3.746 8.424 2 12" stroke="#4a38ee"/>
    <path d="M2 12c1.746-.4 6.123-1 10-1 3.878 0 8.253.6 10 1" stroke="#4a38ee"/>
    <path d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" stroke="#4a38ee"/>
  </svg>
);

const EyeClosed = () => (
  <svg viewBox="0 0 24 20" fill="none" xmlns="http://www.w3.org/2000/svg" width="1.25rem" height="1.25rem">
    <path
...
const VISIBILITY_ORDER: ("public" | "friends" | "private")[] = ["public", "friends", "private"];
...
const TravelLog = ({ authFetch, readOnly = false, countries: externalCountries }: TravelLogProps) => {
  ...
  const [pendingCountry, setPendingCountry] = useState<CountrySuggestion | null>(null);
    ...
    const load = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setCountries((data.countries as Record<string, unknown>[]).map(mapUserCountry));
        }
      } catch {
        // handled
      }
  ...
  const handleSearch = useCallback((value: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await authFetch(`${apiUrl}/countries/search?q=${encodeURIComponent(value)}`);
        if (res.ok) {
          const data = await res.json();
          // Filter out already-added countries
          const existingIds = new Set(countries.map((c) => c.countryId));
          setSuggestions(
            (data.countries as { id: number; name: string; continent: string }[])
  ...
  const confirmAddCountry = async () => {
    ...
    try {
      ...
      if (res.ok || res.status === 201) {
        const data = await res.json();
        const newCountry = mapUserCountry(data.country as Record<string, unknown>);
        setCountries((prev) => [...prev, newCountry].sort((a, b) => {
  ...
  const cycleVisibility = async (country: UserCountry) => {
    ...
    try {
      ...
      if (res.ok) {
        setCountries((prev) =>
          prev.map((c) => (c.id === country.id ? { ...c, visibility: next } : c)),
        );
      }
    } catch {
      // handled
  ...
  return (
    <div className={styles.container}>
          ...
          {showTooltip && (
            <div className={styles.tooltip}>
              <div className={styles.tooltipRow}>
                <EyeOpen />
                <span className={styles.tooltipText}>Travel log visible to friends</span>
              </div>
              <div className={styles.tooltipRow}>
                <EyeHalf />
                <span className={styles.tooltipText}>Country visible, travel log private</span>
              </div>
              <div className={styles.tooltipRow}>
                <EyeClosed />
          ...
          {suggestions.length > 0 && (
            <ul className={styles.suggestions} ref={suggestionsRef}>
              {suggestions.map((s, i) => (
                ...
                >
                  {s.name}
                  <span className={styles.suggestionContinent}>{s.continent}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <div className={styles.detailsForm}>
          ...
          <div className={styles.detailsFields}>
            <label className={styles.detailsLabel}>
              <span>Date visited <span className={styles.optionalTag}>(optional)</span></span>
            ...
            <label className={styles.detailsLabel}>
              <span>Days spent <span className={styles.optionalTag}>(optional)</span></span>
              <input
                type="number"
                className={styles.detailsInput}
                placeholder="e.g. 7"
      ...
      {CONTINENTS.map((continent) => {
        ...
        const displayName = continent === "Antarctica" ? `Antarctica 🥶` : continent;
        ...
        return (
          <div key={continent}>
            <div className={styles.continentRow} onClick={() => toggleContinent(continent)}>
              <div
                className={`${styles.continentArrow} ${isOpen ? styles.continentArrowOpen : styles.continentArrowClosed}`}
              />
              <span className={styles.continentName}>
                {displayName} (<span className={styles.countGreen}>{count}</span>)
              </span>
            </div>
            {isOpen && (
      ...
      {countries.length === 0 && (
        <p className={styles.emptyText}>No countries added yet</p>
      )}
    </div>
  );
};
